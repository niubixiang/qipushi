 ...
 * product_apply_date 申请出库时间 varchar
...
exports.createProduct = (req, res) => {
	...
	const product_all_price = product_inwarehouse_number * 1 * product_single_price
...
exports.applyOutProduct = (req, res) => {
	...
	db.query(sql0, product_out_id, (err, result) => {
		if (result.length > 0) {
			...
			const sql =
				'update product set product_out_status = ?,product_out_id=?,product_out_number=?,product_out_price=?,product_out_apply_person=?,apply_memo=?,product_apply_date= ? where id = ?'
			db.query(sql, [
				product_out_status,
				product_out_id,
				product_out_number,
...
exports.withdrawApplyProduct = (req, res) => {
	const sql =
		'update product set product_out_id = NULL,product_out_status = NULL , product_out_number =NULL,product_out_apply_person=NULL,apply_memo =NULL,product_out_price =NULL,product_apply_date = NULL where id = ?'
	db.query(sql, req.body.id, (err, result) => {
		if (err) return res.cc(err)
		res.send({
			status: 0,
...
exports.auditProduct = (req, res) => {
	...
	if (product_out_status == "同意") {
		...
		db.query(sql, {
			...
			if (err) return res.cc(err)
			const sql1 =
				'update product set product_inwarehouse_number = ?,product_all_price = ?,product_out_status = NULL ,product_out_id = NULL,product_out_number =NULL,product_out_apply_person=NULL,apply_memo =NULL,product_out_price =NULL,product_apply_date = NULL where id = ?'
...
exports.returnApplyProductListData = (req, res) => {
	const number = (req.body.pager - 1) * 10
	const sql =
		`select * from product where product_out_status = "申请出库" || product_out_status = "否决" ORDER BY product_apply_date limit 10 offset ${number} `
	db.query(sql, (err, result) => {
		if (err) return res.cc(err)
		res.send(result)
	})
